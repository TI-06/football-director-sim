...
| 選手管理 | 先発11人、控え7人、フォーメーションへのドラッグ配置、自動編成、主将・PK担当の明示、各種値のソート・絞り込み、負傷、出場停止、疲労、士気、フォーム |
...
| イベント | 選手、医療、メディア、スポンサー、育成に関する選択式イベント。未処理イベント中は追加発生を抑止 |
| キャリア | シーズン表彰、次シーズン開始、年齢更新、シーズン履歴 |
| 保存 | localStorage自動保存、JSONエクスポート・インポート、リセット |
| UI | PCサイドバー、スマホ横スクロールメニュー、ダークスポーツUI、試合実況モーダル、イベント停止付き自動進行 |

## 新しい操作

- スカッド画面では、ピッチ上の選手同士、または選手一覧からピッチのポジションへドラッグして先発を入れ替えられます。スマホやキーボード操作では従来どおり各ポジションの選択欄を利用できます。
- キャプテンは `C`、PKキッカーは `PK` のバッジでピッチ・役割サマリー・選手一覧に表示されます。
- 選手一覧は起用状況、ポジション、OVR、POT、体力、士気、年齢、給与、名前で並べ替えでき、先発・控え・登録外とポジションで絞り込めます。
- 上部の「自動進行」を押すと試合週を連続シミュレーションします。判断イベントの発生、未処理イベント、シーズン終了、または手動停止で止まります。
