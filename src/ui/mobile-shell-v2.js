...
function navItem(id, label, iconName, active) {
  return `<button class="fd2-nav__item ${active ? 'is-active' : ''}" type="button" data-nav="${id}" aria-current="${active ? 'page' : 'false'}">${icon(iconName, 20)}<span>${escapeHtml(label)}</span></button>`;
}
...
export function mobileShellV2(state, currentView, content, uiState = {}) {
  ...
  const menu = `<button class="fd2-nav__item ${active === 'menu' ? 'is-active' : ''}" type="button" data-command="open-game-menu" aria-label="その他のメニュー">${icon('club', 20)}<span>メニュー</span>${unresolved ? `<b class="fd2-nav__badge">${unresolved}</b>` : ''}</button>`;
  return `<div class="fd2-shell">
    <header class="fd2-topbar">
    ...
    <main class="fd2-main" data-current-view="${escapeHtml(currentView)}">${content}</main>
